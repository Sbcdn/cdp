...
use crate::{DBSyncProvider, DataProvider};
...
#[get("/pool/info/{pool_hash}")]
#[openapi(
    id = "api.info.pool.info",
    tags("Pool"),
    summary = "All information about the given stake pool"
)]
pub async fn pool_info(
    pool_hash: String,
    #[filter = "with_auth"] _user_id: String,
) -> Result<Json<serde_json::Value>, Rejection> {
    let pool_info = data_provider()?
        .pool_info(&pool_hash)
        .await
        .map_err(|_| RESTError::Custom("Couldn't find all info of the given stake pool".to_string()))?;

    Ok(rweb::Json::from(json!(pool_info)))
}
